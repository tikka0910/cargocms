...
describe.only('main login page test', () => {
  describe('get index title', () => {
    it('should be get broser title of Login @watch', () => {
      browser.url('https://people.debian.org/~paulliu/ROCid.html');
      console.log(browser.getTitle());
      expect(browser.getTitle()).to.equal('Taiwan ROC ID Generator 台灣身份證字號產生器');
    });
...
*/
        var form = browser.elements('[name="gogogogoo12123"]');
        form.submit();
        expect(browser.getByName().value.not.null );
        done();
      } catch (e) {
        done(e);
      }
